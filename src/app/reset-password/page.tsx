...
const ResetPwdPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      router.push(`/reset-password/${email}?token=${params.get("token")}`);
    } catch (err) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen gap-4">
      ...
      />
      <button onSubmit={handleSubmit} className="bg-primary-700 text-white px-2 py-1 rounded-md hover:bg-primary-500">Send</button>
